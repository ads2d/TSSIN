    ...
    def forward(self, x, test, enhance=1, debug=False):
        ...
        if debug:
            ...
            for i in range(x.size(0)):
                ...
                img_offsets = (offsets_grid[i]).view(1, self.targetH, self.targetW).add_(-v_min).mul_(1./(v_max-v_min))
                #img_offsets = img_offests.add_(-v_min).mul_(1./(v_max-v_min))
                #.data.cpu().add_(-v_min).mul_(1./(v_max-v_min))
